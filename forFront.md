...
현재 백엔드에 카카오 로그인을 위한 구조가 준비되어 있습니다. 구현 시 다음 엔드포인트가 추가됩니다:
...
| GET | `/session` | 세션 확인 | ✅ |
| POST | `/logout` | 로그아웃 | ✅ |
| GET | `/kakao` | 카카오 로그인 (예정) | ❌ |
| GET | `/kakao/callback` | 카카오 콜백 (예정) | ❌ |
...
- **소셜 로그인 준비**: 카카오 로그인을 위한 구조 완비
  - User 엔티티에 `provider`, `providerId` 필드 추가
  - `validateOrCreateSocialUser()` 메서드 준비
  - 카카오 전략 구현 시 바로 사용 가능

### 주요 기능

1. **회원 관리**: CRUD, 회원권, PT 횟수 관리
2. **평가 시스템**: 초기/정기 평가, 능력치 스냅샷 자동 생성
3. **부상 관리**: 부상 이력 및 평가 제한 설정
4. **분석**: 평균 비교, 변화 추이 분석
5. **인사이트**: 대시보드용 통계 및 위험 신호 감지
...
6. **카카오 로그인**: 현재 구조만 준비되어 있으며, 실제 구현은 추후 진행 예정
